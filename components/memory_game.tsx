...
export class MemoryGame extends React.Component<{}, MemoryGameState> {
      ...
      matchPanel = (
        <div className="row-start-1 col-start-1 grid items-center bg-white">
          <div className="grid grid-cols-2 grid-rows-1 items-center">
            <MemoryCard
              card={this.state.cards[this.state.firstCard]}
              onClick={() => undefined}
            ></MemoryCard>
            <MemoryCard
              card={this.state.cards[this.state.secondCard]}
              onClick={() => undefined}
            ></MemoryCard>
          </div>
          <figure className="m-1 p-4">
            ...
            <figcaption className="text-right">
              <cite className="font-medium text-blue-400">
                <a href={cardData["link"]} target="_blank">
                  {cardData["reference"]}, {cardData["translation"]}
                </a>
              </cite>
            </figcaption>
          </figure>
        ...
        <div className="row-start-1 col-start-1 grid items-center bg-white">
          <div className="space-y-8">
            ...
            <p className="text-center">
              <button
                className="bg-blue-500 hover:bg-blue-700 text-white font-bold mx-auto py-2 px-4 rounded"
                onClick={() => this.setState(this.newGame())}
              >
                Noch ein Spiel spielen
              </button>
            </p>
          </div>
        </div>
  ...
  cardClick(cardIndex: number) {
    if (this.state.phase == "openFirst") {
      ...
      if (this.state.cards[cardIndex].side == "back") {
        ...
        const nextPhase =
          this.state.cards[firstCard].name == this.state.cards[secondCard].name
            ? "match"
            : "mismatch";
  ...
  match() {
    ...
    const nextPhase = updatedCards.some((c) => c.side != "found")
      ? "openFirst"
      : "end";
